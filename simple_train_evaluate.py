...
def main():
    ...
    # 训练模型
    model_save_path = os.path.join(os.getcwd(), 'output', 'enhanced_cnn_results', 'enhanced_cnn_model.pth')
    ...
    # 5. 评估模型
    evaluation_results = trainer.evaluate_model(X_test, y_test, model_path=model_save_path)
    ...
    # 6. 保存评估结果
    results_dir = os.path.join(os.getcwd(), 'output', 'enhanced_cnn_results')
    os.makedirs(results_dir, exist_ok=True)
    evaluation_save_path = os.path.join(results_dir, 'enhanced_cnn_evaluation.json')
    
    with open(evaluation_save_path, 'w') as f:
        json.dump(evaluation_results, f, indent=4)
    
    print(f"评估结果已保存到: {evaluation_save_path}")
    
    # 7. 保存训练历史图像
    history_save_path = os.path.join(results_dir, 'enhanced_cnn_training_history.png')
    trainer.plot_training_history(history_save_path)
